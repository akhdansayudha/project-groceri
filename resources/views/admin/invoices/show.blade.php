...
@section('content')
    {{-- LOGIC PARSING DETAILS DI VIEW (Agar tidak perlu ubah Controller) --}}
    @php
            ...
            <div class="bg-gray-50/50 p-8 md:p-12 border-b border-gray-100">
                <div class="flex flex-col md:flex-row justify-between items-start gap-8">
                    ...
                    <div class="text-left md:text-right">
                        ...
                        <p class="text-3xl font-bold text-gray-900 tracking-tight mb-2">#{{ $invoice->invoice_number }}
                        </p>
            ...
            <div class="p-8 md:p-12">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
                    <div>
                        ...
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-full border border-gray-100 overflow-hidden">
                                <img src="{{ $invoice->user->avatar_url ?? 'https://ui-avatars.com/api/?name=' . urlencode($invoice->user->full_name) }}"
                                    class="w-full h-full object-cover">
                            </div>
                            <div>
                                <p class="font-bold text-lg text-gray-900 leading-tight">{{ $invoice->user->full_name }}
                                </p>
                    ...
                    <div>
                        ...
                        @elseif($invoice->status == 'cancelled')
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Cancelled Date
                            </p>
                        ...
                        @else
                            {{-- TAMPILAN DUE DATE --}}
                            <p class="text-[10px] font-bold text-red-500 uppercase tracking-widest mb-2">Due Date</p>
                            @if ($invoice->due_date)
                ...
                <div class="flex flex-col md:flex-row items-center justify-between gap-6 border-t border-gray-100 pt-8">
                    <div class="text-xs text-gray-400 max-w-sm">
                        *Sebagai Admin, Anda dapat membatalkan atau memverifikasi pembayaran ini secara manual jika
                        diperlukan.
                    </div>

                    <div class="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto justify-end">

                        {{-- TOMBOL AKSI (Hanya jika belum lunas) --}}
                        @if (in_array($invoice->status, ['unpaid', 'pending']))
                            ...
                            <button onclick="openPaidModal()"
                                class="px-8 py-3.5 bg-black text-white rounded-xl font-bold text-sm hover:scale-105 transition-transform shadow-xl shadow-black/20 flex items-center justify-center gap-2">
                                <i data-feather="check-circle" class="w-4 h-4"></i>
                                <span>Mark as Paid</span>
                            </button>
                        @endif

                        {{-- Status Info (Jika sudah lunas/batal) --}}
                        @if ($invoice->status == 'paid')
                            <div class="text-sm font-bold text-green-600 flex items-center gap-2">
                                <i data-feather="check-circle" class="w-4 h-4"></i> Pembayaran Terverifikasi
                            </div>
                        @elseif($invoice->status == 'cancelled')
                            <div class="text-sm font-bold text-gray-500 flex items-center gap-2">
                                <i data-feather="x-circle" class="w-4 h-4"></i> Invoice Dibatalkan
                            </div>
                        @endif
                    </div>
                </div>
        ...
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md p-4">
            <div class="bg-white rounded-[2rem] p-8 shadow-2xl transform scale-100 transition-all text-center">
                ...
                <p class="text-gray-500 text-sm mb-8 leading-relaxed">
                    Apakah Anda yakin ingin membatalkan invoice <b>#{{ $invoice->invoice_number }}</b>? Tindakan ini tidak
                    dapat dikembalikan.
                </p>
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <form action="{{ route('admin.invoices.cancel', $invoice->id) }}" method="POST" class="w-full">
                        @csrf
                        @method('PUT')
                        <button type="submit"
    ...
    <div id="paidModal" class="fixed inset-0 z-50 hidden fade-in">
        <div class="absolute inset-0 bg-black/40 backdrop-blur-sm transition-opacity" onclick="closePaidModal()"></div>
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md p-4">
            <div class="bg-white rounded-[2rem] p-8 shadow-2xl transform scale-100 transition-all text-center">
                <div
                ...
                <h3 class="text-2xl font-bold text-gray-900 mb-2 tracking-tight">Confirm Payment?</h3>
                <p class="text-gray-500 text-sm mb-8 leading-relaxed">
                    Sistem akan menandai invoice ini sebagai <b>LUNAS</b> dan otomatis menambahkan token ke wallet user.
                </p>
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <form action="{{ route('admin.invoices.paid', $invoice->id) }}" method="POST" class="w-full">
                        @csrf
                        @method('PUT')
                        <button type="submit"
                            class="w-full px-6 py-3.5 bg-black text-white rounded-xl font-bold text-sm hover:bg-gray-800 transition-colors shadow-lg">
                            Konfirmasi
                        </button>
                    </form>
                </div>
